...
| 2.4L | Chip adornment cluster rework (sibling-of-trigger edit/delete/checkbox/lock, hover-reveal, flush-left title) + `CalendarEvent.locked` / `onToggleLock` (blocks move/resize only); UTC-display bug fixes across chip/popover time display, `buildIso`/`toTimeSlot`, and `WeekCalendarView` day-membership/overnight-splitting — all now LOCAL wall-clock (BREAKING: `buildIso`/`toTimeSlot` semantics flip from UTC-parts to local-wall-clock) | On `feat/w2.4L-chip-lock-utc` — commits local, not yet pushed |
| 2.5L | `WeekCalendarView` gains `onWeekChange` (nav callback, fires on week change only, Sunday local-ISO payload) + `resyncToken` (identity change re-seeds `localEvents` from `events` prop, deferred mid-drag, never touches displayed week) — both optional, backward compatible                                                                                                                                                                 | On `feat/w2.5L-week-resync` — commits local, not yet pushed   |
| 2.6L | NumberInput `allowEmpty` (opt-in `value`/`onChange` widen to `number \| undefined`); repeat-count fields blank by default — open-ended recurring series (`recurrenceCount: undefined`) from the drag-create popover and ActivityFormDialog (BREAKING: `TaskTimeFieldsProps.recurrenceCount`/`onRecurrenceCountChange` widen to `number \| undefined`); `WeekCalendarView` chip-popover edit fan-out preserve for server-materialized rows with no `recurrenceDays` | On `feat/w2.6L-open-ended-count` — PR pending                 |
...
- **SB_PORT rule (2026-07-07):** local e2e/Storybook reruns MUST use a fresh port, never reuse `:6006` — `playwright.config.ts` reads `SB_PORT` to override the default (`SB_PORT=6011 pnpm playwright test` / `SB_PORT=6011 pnpm storybook dev -p 6011`). Hit twice in one wave-cycle (2026-06-17, 2026-07-07): a leftover session's :6006 (and sometimes :6008) silently serves stale stories to a second worktree's e2e run, so a passing suite proves nothing about the code actually under test. Check `netstat -ano | grep -E ":6006|:6008"` before trusting a local `just e2e`/`just check` run; pick an unused port for `SB_PORT` rather than killing another session's server.
- **Browser `timezoneId` pinned (2026-07-07):** `playwright.config.ts`'s `use.timezoneId` is pinned to `'America/New_York'` — CI (`ubuntu-latest`) otherwise defaults to UTC, and any TZ-sensitive story (calendar day-membership, overnight splitting) renders differently there than on a developer's own machine. A story that looks correct locally can still fail its axe entry in CI purely from the host TZ shifting which local calendar day an instant falls on. Never rely on the host/CI machine's timezone for a story's rendered output — construct TZ-sensitive fixtures against the pinned zone, and note the pin in the story's own docs description if its thesis depends on it.
