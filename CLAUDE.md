...
| 2.2L | Calendar time fixes: TimeGutterLabel SSR/occlusion fixes, WeekCalendarView `use24h` + `dayWindows`, drag-slop threshold fix, CalendarEvent `completed` + chip/view complete-toggle, SleepBand `interactive` prop removed; ActivityFormDialog rich-field parity (BREAKING: `ActivityType` drops `'habit'`) | On `feat/w2.2L-calendar-time` — In PR pending |
