...
- 093e0dc: CarsickYak theme preset: `themes/carsickyak.css` scoped semantic-token override (`.theme-carsickyak` light / `.theme-carsickyak.dark`), exported subpath. Ember primary + pine secondary on neutral grounds; new palette-extra vars (`--ember`, `--pine`, `--gold`, `--sage`, `--navy`, `--yak-brown`) and a `--font-serif` slot.
...
- 3fdbaae: Form shell wave (slot-W2): apps own form interiors.
...
- 6e407b0: Slot generalization W1 — extension convention + cheap evidenced slots: ChatPanel `header`/`actions`/`modal`/aside passthrough, ChatFab `icon`/`label`/`position`, EmptyState + HeroCard `renderCta`, SleepToggle `label`, Sidebar controlled `collapsed`/`onCollapsedChange`, Combobox `renderOption`, DatePicker/DateRangePicker `renderTrigger` + Calendar `classNames`/`components` pass-through, CountUp `format`, TopBar/PageHeader/SidebarBrand `title` widened to ReactNode. All additive.
...
- 6454045: WeekCalendarView: controlled create-popover reopen + duplicate-from-event
...
- f7a2e94: Wave 3L — Activity model: recurring creation/editing moves to app-side activity schedules, calendar events link to activities, and a new template + event-form dialog pair replaces the old type-morphing form.
...
- 9655770: `WeekCalendarView` gains `onWeekChange` and `resyncToken` — both optional, backward compatible.
